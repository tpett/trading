...
def make_spy_closes(
    ...
    drift: float = 0.0003, vol: float = 0.01,
) -> pd.Series:
    ...
    data/equities-tiingo/SPY cache. vol=0.0 gives a deterministic
    constant-drift benchmark: the re-anchored checks score the LO-minus-SPY
    active series, and unit tests that plant a signal in the LO leg need the
    benchmark side quiet enough not to drown it over short fixture windows."""
    rng = np.random.default_rng(seed)
    idx = pd.date_range(start, periods=periods, freq="B", tz="UTC")
    rets = drift + rng.normal(0.0, vol, size=periods)
