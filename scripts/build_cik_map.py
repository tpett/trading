...
# FSDS-derived intervals for delisted/acquired symbols company_tickers.json no
# longer lists (generated by scripts/build_cik_map_historical.py, committed and
# reviewed like RENAMES); merged into every regeneration by merge_historical().
HISTORICAL = ROOT / "src" / "trading" / "fundamentals" / "cik_map_historical.csv"
...
def merge_historical(
    rows: list[tuple[str, int, str, str]], historical: pd.DataFrame
) -> tuple[list[tuple[str, int, str, str]], list[str]]:
    """Fold the committed historical (FSDS-derived) intervals into a freshly
    built row set so regeneration never silently drops delisted symbols.

    A historical row is skipped -- returned in the second element for the
    caller to report -- if its symbol is in EXCLUSIONS (recycled tickers stay
    unmapped) or its interval overlaps an interval build_rows already emitted
    for the same symbol (the live resolution wins). Output is re-sorted by
    (symbol, start): deterministic, and a multi-interval symbol (e.g. DOC)
    reads in chronological order."""
    spans: dict[str, list[tuple[str, str]]] = {}
    for symbol, _cik, start, end in rows:
        spans.setdefault(symbol, []).append((start, end or "9999"))
    merged = list(rows)
    skipped: list[str] = []
    for row in historical.itertuples():
        hist_end = row.end or "9999"
        overlaps = any(
            row.start < end and start < hist_end for start, end in spans.get(row.symbol, [])
        )
        if row.symbol in EXCLUSIONS or overlaps:
            skipped.append(row.symbol)
            continue
        merged.append((row.symbol, int(row.cik), row.start, row.end))
    return sorted(merged, key=lambda r: (r[0], r[2])), skipped
...
def main() -> None:
    ...
    if HISTORICAL.exists():
        historical = pd.read_csv(HISTORICAL, comment="#", dtype=str).fillna("")
        rows, skipped = merge_historical(rows, historical)
        mapped_symbols = {r[0] for r in rows}
        unmapped = [s for s in unmapped if s not in mapped_symbols]
        print(
            f"merged {len(historical) - len(skipped)} historical intervals "
            f"from {HISTORICAL.name}" + (f" (skipped: {sorted(skipped)})" if skipped else "")
        )
